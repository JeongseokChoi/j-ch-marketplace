...
(길면 요청 목록, 열린 todo, 마지막 요청)를 받아 흐름을 파악하고, 이전 요청 아래에 덧붙이거나
남은 todo 를 완료 처리할 수 있다. 이어받는 범위는 이 PC 에 남은 세션 상태이고,
...
├── 인증 미들웨어 3단계 분리                        h2  요청 (note: 날짜·시각·디렉터리)
│   ├── ❝ 요청 원문 ❞                               quote-block
...
| 타입 | 쓰임 |
|---|---|
| `h2` `h3` `h1` | 요청, 요청 안의 큰 주제, 여러 요청을 묶는 구획 |
...
열린 todo 가 없으면 이 세션의 마지막 제목이다. 이어받은 노드에는 붙이지 않는다. description 이 없는 읽기 도구(Read·Grep 등)와
...
| PreToolUse 훅 `guard` | 기록 중인 세션의 메인 Claude 가, root 또는 이 세션에서 만들었거나 이어받은 노드 아래에만 쓰도록 검사. 범위 안이면 권한 확인 없이 허용 |
...
지정한 노드가 세션 보관함이 아니라 작업 흐름 하나가 되어, 세션마다 제목(h1)을 만들지 않고
요청(h2)을 노드 바로 아래에 이어 쓴다. 2.x 로 쓴 노드도 같은 노드로 시작하면 이어받는다
