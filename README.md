...
├── **인증 미들웨어 3단계 분리**                    bullets 굵게  요청 (note: 날짜·시각·디렉터리)
│   ├── ❝ 요청 원문 ❞                               quote-block
...
| 타입 | 쓰임 |
|---|---|
| `bullets` | 발견, 결정, 막힌 점. 요청과 요청 안의 큰 주제는 굵게 |
...
열린 todo 가 없으면 이 세션에서 root 바로 아래에 마지막으로 만든 요청이다. 이어받은 노드에는 붙이지 않는다. description 이 없는 읽기 도구(Read·Grep 등)와
