...
└── ## my-api · 2026-09-21 19:30          세션 (종료 시 체크)
    ...
    ├── ☑ 19:36 🤖 Explore: Find JWT verify callers · 2분   서브에이전트
    │       note: 결과: … / 지시: …
    ├── ☐ 19:41                              턴 (진행 중: 시각만)
    │       note: 전체 프롬프트
    └── ⏹ 종료 · clear · 22분
...
`🤖` 항목은 Claude 가 띄운 서브에이전트다. 에이전트는 턴이 끝난 뒤에도 일할 수 있으므로 턴 아래가
아니라 턴과 같은 단계에 시작 시각과 함께 둔다. 그래서 일하는 동안은 체크 안 된 항목으로 보이고,
턴은 에이전트와 상관없이 응답이 끝나면 체크된다. 에이전트를 띄울 때 적은 설명이 제목이 되고,
끝나면 소요 시간과 함께 체크되며 노트 첫 줄에 **에이전트가 알아 온 결과**가 남는다.
