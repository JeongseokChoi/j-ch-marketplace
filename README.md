...
└── ## my-api · 2026-09-21 19:30          세션 (종료 시 체크)
    ...
    ├── ☑ 19:36 🤖 Explore: Find JWT verify callers · 2분   서브에이전트 (진행 중: · 진행 중)
...
소요 시간과 함께 체크되며 노트 첫 줄에 **메인 세션의 응답(보고)** 이 남는다. 에이전트는 턴이
끝난 뒤에도 일할 수 있으므로 턴 아래가 아니라 턴과 같은 단계에 시작 시각과 함께 둔다. 그래서
일하는 동안은 체크 안 된 항목으로 보이고, 턴은 에이전트와 상관없이 응답이 끝나면 체크된다.
서브에이전트가 다시 띄운 에이전트는 기록하지 않는다.
