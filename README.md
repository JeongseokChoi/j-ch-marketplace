...
이상일 때만 붙는다. 에이전트의 보고나 완료 알림처럼 Claude Code 가 스스로 넣는 턴은 사용자
요청이 아니므로 턴으로 기록하지 않는다.
...
`🤖` 항목은 Claude 가 서브에이전트에게 맡긴 일이다. 일은 서브에이전트가 하고 기록은 메인 세션이
한다. 메인 세션이 에이전트에게 맡기는 순간 `… · 진행 중` 항목이 생기고, 에이전트의 보고를 받아
응답하면 `· 진행 중` 이 소요 시간으로 바뀌며 노트 첫 줄에 **그 응답(메인 세션의 보고)** 이 남는다. 에이전트는
턴이 끝난 뒤에도 일할 수 있으므로 턴 아래가 아니라 턴과 같은 단계에 시작 시각과 함께 둔다.
턴 제목은 에이전트와 상관없이 응답이 끝나면 채워진다. 세션이 끝날 때까지 일하던 에이전트는
`· 중단됨` 으로 표시된다. 서브에이전트가 다시 띄운 에이전트는 기록하지 않는다.
