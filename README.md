...
요청 전문은 노트에 있다. 응답을 찾지 못하면 요청 앞부분을 제목으로 쓴다. 소요 시간은 1분
...
턴 아래의 `▸` 메모는 `session-log` 스킬로 남긴 것이다.
...
적히고, 끝나면 제목은 답변 첫 문장, 노트는 답변 전문이 된다. 어느 보고가 계기였는지는 진행 중일
...
도구 호출(실행한 명령, 수정한 파일 등)은 기록하지 않는다.
