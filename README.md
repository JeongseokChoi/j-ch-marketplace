...
Claude Code 세션의 작업 과정을 사용자가 지정한 Workflowy 노드 아래에 Claude 가 정리해 기록한다.
...
기록은 세션마다 직접 시작한다. 시작하지 않은 세션은 아무것도 기록하지 않는다.

```
/workflowy:session-log daa0961ddeee                        # 이 노드 아래에 기록 시작 (short id)
/workflowy:session-log https://workflowy.com/#/daa0961ddeee  # URL 도 된다
/workflowy:session-log                                     # 현재 상태: root, 만든 노드, 지금 작업 중인 노드
/workflowy:session-log stop                                # 기록 중단 (쓴 노드는 그대로)
...
```
📁 AI 작업 로그                                   ← 지정한 노드 (건드리지 않음)
└── j-ch-marketplace · 2026-09-23 14:05            h1  세션
    ├── 인증 미들웨어 3단계 분리                    h2  요청
    │   ├── ❝ 요청 원문 ❞                           quote-block
...
|---|---|
| `h1` `h2` `h3` | 세션, 요청, 큰 주제 |
| `todo` | Phase, 단계, 하위 작업. 끝날 때마다 완료 처리 |
| `bullets` | 발견, 결정, 막힌 점 |
| `p` | 계획, 결과 같은 단락 |
...
지금 작업 중인 노드는 트리 순서로 완료되지 않은 첫 todo(그 안에 열린 todo 가 있으면 가장 안쪽),
열린 todo 가 없으면 마지막 제목이다. description 이 없는 읽기 도구(Read·Grep 등)와
...
| 구성 | 역할 |
|---|---|
| MCP 서버 `workflowy` (`scripts/mcp.py`) | Claude 가 쓰는 도구 `create`(노드 추가), `complete`(todo 완료). 수정·삭제 도구는 없다 |
| PreToolUse 훅 `guard` | 기록 중인 세션의 메인 Claude 가, root 또는 이 세션에서 만든 노드 아래에만 쓰도록 검사. 범위 안이면 권한 확인 없이 허용 |
| PostToolUse 훅 `track` | 만든 노드와 완료 여부를 세션 상태에 기록 |
| PreToolUse 훅 `step` | 도구 실행을 지금 작업 중인 노드 아래에 `▹` 로 추가 |
| UserPromptSubmit 훅 `prompt` | `/workflowy:session-log` 인자 처리. 기록 중이면 요청마다 기록 지침을 한 줄로 상기 |
| SessionStart 훅 | 대화 압축·재개 뒤 기록 중인 root 와 지금까지 만든 노드(id 포함)를 Claude 에게 다시 알림 |

- 세션 상태는 `${CLAUDE_PLUGIN_DATA}/state/<session id>.json` 에 있다.
- `/clear` 는 새 세션이 되므로 기록이 끊긴다. 이어서 기록하려면 다시 시작한다.
  같은 노드를 주면 그 아래에 새 세션 제목부터 다시 쓴다.
...
- Workflowy API 는 layoutMode 값을 검증하지 않는다 (모르는 값은 bullet 으로 그려진다).
  MCP 서버가 위 타입만 받는다. 코드 블록은 layoutMode 대신 ``` 로 감싸 보내야 여러 줄이 한 블록에 들어간다.
